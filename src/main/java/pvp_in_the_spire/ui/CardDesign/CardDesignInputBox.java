...
public class CardDesignInputBox extends InputBoxWithLabel
    implements InputBoxChange, CardModifyItem {

    //负责管理的核心卡牌
    public AdaptableCard mainCard = null;
    //卡牌被修改时产生的回调函数
    ...
    public void onInputBoxChanged(String text) {
        //调用相应的卡牌修改回调
        if(this.modifyEvent != null && this.mainCard != null)
        {
    ...
    @Override
    public void registerCard(AdaptableCard card) {
        //记录卡牌
        this.mainCard = card;
        //更新文本显示
        this.inputBox.textField = this.modifyEvent.initCardInfo(card);
    }
}
